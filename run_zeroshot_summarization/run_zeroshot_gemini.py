...
for i in tqdm(range(annotations.shape[0])):
  dx_list.append(client.models.generate_content(model='gemini-2.0-flash', contents=dx_prompt+annotations['report'][i]).text)
  time.sleep(5)
  stage_list.append(client.models.generate_content(model='gemini-2.0-flash', contents=stage_prompt+annotations['report'][i]).text)
  time.sleep(5)
  aspirate_disease_list.append(client.models.generate_content(model='gemini-2.0-flash', contents=aspirate_disease_prompt+annotations['report'][i]).text)
  time.sleep(5)
  pblood_disease_list.append(client.models.generate_content(model='gemini-2.0-flash', contents=pblood_disease_prompt+annotations['report'][i]).text)
  time.sleep(5)
  flow_summary_list.append(client.models.generate_content(model='gemini-2.0-flash', contents=flow_summary_prompt+annotations['report'][i]).text)
  time.sleep(5)
  flow_blasts_list.append(client.models.generate_content(model='gemini-2.0-flash', contents=flow_blasts_prompt+annotations['report'][i]).text)
  time.sleep(5)
 
# Add summaries to annotations file
annotations['dx_gemini'] = dx_list
annotations['stage_gemini'] = stage_list
annotations['aspirate_disease_gemini'] = aspirate_disease_list
annotations['pblood_disease_gemini'] = pblood_disease_list
annotations['flow_gemini'] = flow_summary_list
annotations['flow_blasts_gemini'] = flow_blasts_list
annotations.to_csv('./data/annotations/gemini_annotations.csv')
